...
let print chn msg = 
  ...
  let rec prTree ind node = 
    if !do_use_performance_counters then 
      (Printf.fprintf chn "%s%-20s          %8.5f s (%8.5f s)\n" 
         (String.make ind ' ') node.name node.time node.max)
    else
      (Printf.fprintf chn "%s%-20s          %6.3f s (%6.3f s)\n" 
         (String.make ind ' ') node.name node.time node.max);
...
let repeattime limit str f arg = 
  ...
  let stat : t = 
    ...
    let rec loop = function
      ...
      | _ :: rest -> loop rest
      | [] -> 
          let nw = {name = str; time = 0.0; max = 0.0; sub = []} in
  ...
  let rec loop count = 
    ...
    if diff < limit then
      ...
      let x = (diff /. float(count)) in
      stat.time <- stat.time +. x;
      stat.max <- if x > stat.max then x else stat.max;
